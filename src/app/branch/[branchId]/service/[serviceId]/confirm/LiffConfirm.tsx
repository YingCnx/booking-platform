...
// ดึง LINE profile แล้วกรอกชื่อให้อัตโนมัติ
// ==============================================
...
export function LiffConfirm({ branchId, serviceId, time, date }: Props) {
  ...
  useEffect(() => {
    ...
    script.onload = async () => {
      try {
        ...
        setIsInLine(window.liff.isInClient())
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // ส่งไป server action ผ่าน fetch (เพราะต้องรอ LIFF)
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {lineUserId && (
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl px-4 py-3">
          {avatar && <img src={avatar} alt="" className="w-9 h-9 rounded-full" />}
          <div>
            <div className="text-xs text-green-700 font-medium">เชื่อมต่อ LINE แล้ว</div>
            <div className="text-sm text-green-900 font-semibold">{name}</div>
          </div>
        </div>
      )}
