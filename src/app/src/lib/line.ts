...
// ==============================================
// Admin link — ผ่าน groupId
// ==============================================
// ต้อง verify userId + groupId ที่ /admin-login

function adminUrl(groupId: string, bookingId?: string) {
  const base = process.env.NEXT_PUBLIC_SITE_URL ?? ''
  // ส่ง groupId + (optional) bookingId เพื่อพากลับมาตอน verify เสร็จ
...
export function buildAdminNotifyFlex(data: {
  ...
  price: number
  bookingId: string
  adminGroupId: string  // ✅ ใส่ groupId เพื่อให้ admin verify ตอนกดลิงก์
  ...
  const footerButtons: any[] = [
    {
      ...
      color: '#1C1C1E',
      height: 'sm',
      action: { type: 'uri', label: 'Dashboard', uri: adminUrl(data.adminGroupId, data.bookingId) },
    },
  ]

  if (data.lineUserId) {
    const contactUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/admin-login?groupId=${encodeURIComponent(data.adminGroupId)}&next=${encodeURIComponent('/admin/contact?bookingId=' + data.bookingId)}`
    footerButtons.push({
      type: 'button',
      style: 'secondary',
      height: 'sm',
