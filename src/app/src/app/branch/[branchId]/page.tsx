...
export default async function BranchPage({ params }: Props) {
  ...
  if (!session) redirect(`/liff?next=/branch/${branchId}`)

  const supabase = await createClient()

  // ✅ ดึง branch + เช็คว่าอยู่ใน shop ของลูกค้า
  const { data: branch } = await supabase
    .from('branches')
    .select('id, name, address, open_time, close_time, shop_id')
    .eq('id', branchId)
    .single()

  // ✅ ถ้า branch ไม่ใช่ของ shop ของลูกค้า → ไม่ให้ดู
  if (!branch || branch.shop_id !== session.shopId) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50 px-6">
        <div className="text-center">
          <div className="text-5xl mb-3">🚫</div>
          <p className="text-gray-500 text-sm">ไม่พบสาขานี้</p>
          <Link href="/branch" className="mt-4 inline-block text-sm underline text-gray-900">
            กลับไปเลือกสาขา
          </Link>
        </div>
      </main>
